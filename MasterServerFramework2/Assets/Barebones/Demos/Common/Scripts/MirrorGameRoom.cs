...
/// </summary>
public class MirrorGameRoom : NetworkBehaviour
{
    public static SpawnTaskController SpawnTaskController;
    
    public HelpBox _header = new HelpBox()
    {
        Text = "Waits for the Mirror game server to start," +
    ...
    public BmLogger Logger = Msf.Create.Logger(typeof(MirrorGameRoom).Name);

    protected Dictionary<int, NetMsfPlayer> PlayersByPeerId;
    protected Dictionary<string, NetMsfPlayer> PlayersByUsername;
    protected Dictionary<int, NetMsfPlayer> PlayersByConnectionId;

    public event Action<NetMsfPlayer> PlayerJoined;
    public event Action<NetMsfPlayer> PlayerLeft;
    ...
    protected virtual void Awake()
    {
        ...
        PlayersByPeerId = new Dictionary<int, NetMsfPlayer>();
        PlayersByUsername = new Dictionary<string, NetMsfPlayer>();
        PlayersByConnectionId = new Dictionary<int, NetMsfPlayer>();
    ...
    {
        NetworkServer.RegisterHandler<TokenMessage>(HandleReceivedAccess);

        // Find the manager, in case it was inaccessible on awake
        NetworkManager = NetworkManager ?? FindObjectOfType<NetworkManager>();

        // The Mirror server is started, we need to register a Room
    ...
    {
        NetworkServer.UnregisterHandler<TokenMessage>();
        base.OnStopServer();
    }
    ...
    protected virtual void HandleReceivedAccess(NetworkConnection conn, TokenMessage netmsg)
    {
        var token = netmsg.token;
        ...
        Controller.ValidateAccess(token, (validatedAccess, error) =>
        {
            ...
            Msf.Server.Auth.GetPeerAccountInfo(validatedAccess.PeerId, (info, errorMsg) =>
            {
                ...
                var player = new NetMsfPlayer(conn, info);
    ...
    protected virtual void OnPlayerJoined(NetMsfPlayer player)
    {
        // Add to lookups
    ...
    protected virtual void OnPlayerLeft(NetMsfPlayer player)
    {
        // Remove from lookups
    ...
    public NetMsfPlayer GetPlayer(string username)
    {
        NetMsfPlayer player;
    ...
    public Dictionary<string, NetMsfPlayer> GetPlayers()
    {
